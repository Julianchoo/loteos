    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {project.name} — modificá los datos y guardá los cambios.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 py-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                ...
                onChange={(e) => handleChange("totalLots", e.target.value)}
                disabled={loading}
              />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="minCashDown">Anticipo mín. (USD)</Label>
              <Input
                id="minCashDown"
                value={form.minCashDown ?? ""}
                onChange={(e) => handleChange("minCashDown", e.target.value)}
                disabled={loading}
              />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="maxFinancingMonths">Plazo máx. (meses)</Label>
              <Input
                ...
                onChange={(e) =>
                  setForm((prev) => ({
                    ...prev,
                    maxFinancingMonths: e.target.value === "" ? null : parseInt(e.target.value, 10),
                  }))
                }
                disabled={loading}
              />
